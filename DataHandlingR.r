...
FileChoose_CBC_2019 <- read.table(file = ______,
                                  header = ______,
                                  sep = ______)
...
CBC_2019 <- read.table(file = ______, 
                        sep = "\t",  
                       header = TRUE) 
...
# Once openxlsx is installed, load it with library()
...
# CBC_2022 <- read.table("ChristmasBirdCount_2022_SelectCities.txt", 
#                          header = FALSE, sep = "-")
# CBC_2022 <- read.csv("ChristmasBirdCount_2022_SelectCities.txt", 
#                        header = FALSE)
...
# Load the dplyr package with library()
library(dplyr)  
...
CBC_2019_2020 <- ______(CBC_2020, CBC_2019) 
...
Birdspecific <- ______(Birdspecific_1, Birdspecific_2)
head(Birdspecific)
...
CBC_wScinames <- ______(CBC_2019_2022, Scinames, by = "Common_Name")
head(CBC_wScinames) 
...
head(CBC_wAvonet)
CBC_data <- ______(CBC_wAvonet, # Data frame to subset columns from
...
filter_numeric <- ______(CBC_data, # Data frame to subset rows from
...
filter_and <- filter(CBC_data, # Data frame to subset rows from
						Count < 5 ______ !is.na(Flags)) # Subset criteria
head(filter_and)
...
# How many omnivore (Trophic.Level ==  "Omnivore") birds
# were found in Peterborough's 2020 count?
# Present the results in a simplified data frame showing only:
# Scientific name, common name, and count
# Order the simplified data frame by decreasing count

# Match filter(), select(), and arrange() with the corresponding code

# Step 1: Reduce data set to counted omnivore birds counted in 2020
Step1 <- filter(CBC_data_arranged, # Data frame to subset rows from
                Trophic.Level ==  "Omnivore" & Year ==  2020 & 
                  Location == "Peterborough" & Count > 0) # Subset criteria

# Step 2: Reduce the data frame to only columns of interest
Step2 <- select(Step1, # Data frame to remove columns from
				Scientific_Name, Common_Name, Count) # Columns selected by name

# Step 3: Arrange by descending count
Step3 <- arrange(Step2, # Data frame to sort
				desc(Count)) # Column to sort by
Step3
nrow(Step3)
...
mutate_newcol0 <- ______(CBC_data_arranged, 
...
Step3_pipe <- CBC_data_cleaned______ 
...
CBC_data_cleaned %>%
  ...
  ______(Location, Year) %>% 
  # group_by() %>% mutate(): compute new variables by group
  ______(Total_Count = sum(Count, na.rm = TRUE)) %>% # total number of birds observed 
  data.frame() %>% head()
...
write.table(x = CBC_summary, 
            ...
            col.names = ______, row.names = ______, sep = ______)
